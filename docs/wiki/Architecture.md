...
└── AgentsMonitor/               app target — thin SwiftUI
...
`AgentsMonitorKit`, a local Swift package with zero dependency on AppKit or
...
- **Local accounts** (`AccountKind.local`) — credentials are owned by Claude Code on this Mac.
  ...
  against that profile occasionally, its token stays fresh without Agents Monitor's help.
- **Remote accounts** (`AccountKind.remote`) — credentials are pasted in and owned entirely by
  Agents Monitor, stored in its own keychain item. There's no CLI on this machine holding the
  same lineage, so Agents Monitor is free to run the OAuth refresh grant
...
things make Agents Monitor's reads and writes silent:

- **Reads:** `SecKeychainSetUserInteractionAllowed(false)` turns the would-be consent modal into
  a clean `errSecAuthFailed` instead of a hang. On that failure, Agents Monitor falls back to
...
- **Writes** (remote accounts only, to Agents Monitor's *own* items): `SecItemUpdate` first,
