# Claude Monitor — Implementation Plan (final, rev 2)
...
`objectVersion = 77` → `PBXFileSystemSynchronizedRootGroup`: source files are **never** listed in the pbxproj. Dropping a `.swift` file into `ClaudeMonitor/` is sufficient; no project edit when adding files.

## File layout

Root `/Users/roypadina/Code/Padina/ClaudeMonitor`.

### Copied from LanGuard (8 files, sed-renamed — no authoring)

| Path | Responsibility |
|---|---|
| `Config/Shared.xcconfig` | bundle id `com.roy.claudemonitor`, `LSUIElement=YES`, `MACOSX_DEPLOYMENT_TARGET=14.0`, ad-hoc `CODE_SIGN_IDENTITY = -` |
| `Config/{Debug,Release,Tests}.xcconfig` | thin `#include` wrappers |
| `Config/ClaudeMonitor.entitlements` | `app-sandbox = false` (other-app keychain + SMAppService) |
| `ClaudeMonitor.xcodeproj/project.pbxproj` | app target + UITests stub + package product dependency |
| `…/project.xcworkspace/contents.xcworkspacedata`, `…/xcshareddata/xcschemes/ClaudeMonitor.xcscheme` | shared scheme so `-scheme ClaudeMonitor` works from CLI |
| `ClaudeMonitor.xcworkspace/contents.xcworkspacedata` | ties `group:ClaudeMonitorKit` + `container:ClaudeMonitor.xcodeproj` — **this resolves the local package**; all builds must use `-workspace` |
| `ClaudeMonitor/ClaudeMonitor.xctestplan` | runs `ClaudeMonitorKitTests` under `xcodebuild test` |
| `ClaudeMonitorUITests/ClaudeMonitorUITests.swift` | generated stub — **leave it**; removing the target is pbxproj surgery for no gain |

Rename order matters (longer tokens first — they contain the shorter ones):
```
LanGuardFeatureTests -> ClaudeMonitorKitTests
LanGuardPackage      -> ClaudeMonitorKit
LanGuardFeature      -> ClaudeMonitorKit
LanGuard             -> ClaudeMonitor
languard             -> claudemonitor
```
Apply to file contents *and* file/dir names. Then hand-edit `Shared.xcconfig`: `PRODUCT_DISPLAY_NAME = Claude Monitor`, `MARKETING_VERSION = 1.0.0`, `CURRENT_PROJECT_VERSION = 1`; rewrite the entitlements comment to say why *this* app is unsandboxed.

### `ClaudeMonitorKit/` — local SPM package, all logic
...
| `Tests/ClaudeMonitorKitTests/ClaudeMonitorKitTests.swift` | **one** file, five `XCTestCase` classes | ~230 |
...
public enum KeychainService {
    ...
    public static func ownedServiceName(accountId: UUID) -> String          // "ClaudeMonitor-account-<uuid>"
}
...
```bash
swift test --package-path /Users/roypadina/Code/Padina/ClaudeMonitor/ClaudeMonitorKit
```
MCP: `swift_package_test({ packagePath: "/Users/roypadina/Code/Padina/ClaudeMonitor/ClaudeMonitorKit" })`

**Session defaults** (set once; `build_macos` / `test_macos` read these):
```
session_set_defaults({
  workspacePath: "/Users/roypadina/Code/Padina/ClaudeMonitor/ClaudeMonitor.xcworkspace",
  scheme: "ClaudeMonitor",
  configuration: "Release",
  derivedDataPath: "/Users/roypadina/Code/Padina/ClaudeMonitor/build"
})
```
Then `list_schemes` to confirm the sed-rename produced scheme `ClaudeMonitor`, then `build_macos({})`, `get_mac_app_path({})`, `test_macos({})`.

**CLI fallbacks** — note `-workspace`, never `-project`, or the package won't resolve:
```bash
cd /Users/roypadina/Code/Padina/ClaudeMonitor
xcodebuild -workspace ClaudeMonitor.xcworkspace -scheme ClaudeMonitor -configuration Debug   -derivedDataPath build build
xcodebuild -workspace ClaudeMonitor.xcworkspace -scheme ClaudeMonitor -configuration Release -derivedDataPath build build
xcodebuild -workspace ClaudeMonitor.xcworkspace -scheme ClaudeMonitor -configuration Debug   test
```
...
codesign -dv /Applications/ClaudeMonitor.app     # verify ad-hoc signature survived
