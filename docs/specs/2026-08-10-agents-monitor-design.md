...
Native macOS menubar app monitoring Claude Code usage limits across **N accounts** (each = a `CLAUDE_CONFIG_DIR`), with configurable desktop + ntfy phone alerts. Lives at `~/Code/Padina/AgentsMonitor`.
...
```
AgentsMonitor.xcodeproj
├── AgentsMonitorKit/            local SPM package — all logic, headless-testable
...
1. `swift test` green in AgentsMonitorKit (parsing both real fixture shapes — with/without spend limit, derivation, alert-engine transitions).
